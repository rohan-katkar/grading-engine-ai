...
class User(Base):
    ...
    submissions = relationship(
        ...
        foreign_keys="StudentSubmission.student_id",
...
class HumanReview(Base):
    ...
    reviewer_id = Column(GUID(), ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True)
...
# %% [default users]
def seed_default_users() -> Dict[str, uuid.UUID]:
    session = SessionLocal()
    default_users = [
        {"full_name": "Alice Student", "email": "student@evalai.org", "role": "STUDENT", "password": "StudentPassword123!"},
        {"full_name": "Bob Reviewer", "email": "reviewer@evalai.org", "role": "EXAM_REVIEWER", "password": "ReviewerPassword123!"},
        {"full_name": "Carol Creator", "email": "creator@evalai.org", "role": "EXAM_CREATOR", "password": "CreatorPassword123!"},
        {"full_name": "Dave Admin", "email": "admin@evalai.org", "role": "ADMIN", "password": "AdminPassword123!"},
    ]
        ...
        for u in default_users:
            ...
            if not existing:
                user_obj = User(
                    ...
                    email=u["email"],
                    password_hash=hash_password(u["password"]), # Clean salted hash
                    role=u["role"]
                )
                session.add(user_obj)
                session.flush()
                user_ids[u["role"]] = user_obj.user_id
            else:
                user_ids[u["role"]] = existing.user_id
        session.commit()
        print("✅ Seeded default RBAC users with salted password hashes.")
        return user_ids
...
def log_grading_run(state: Dict[str, Any], submission_id: Optional[Any] = None) -> Optional[uuid.UUID]:
    """
    Strictly logs a grading run. Requires a valid, registered student_id.
    Raises ValueError if student_id is missing or not registered in the database.
    """
    session = SessionLocal()
    try:
        ...
        if not student_user:
            raise ValueError(f"❌ Logging failed: No registered user found for student_id '{student_id}'.")
        ...
        # 5. Queue Human Review if Flagged
        if final_status == "NEEDS_HUMAN_REVIEW":
            review_entry = HumanReview(
                review_id=uuid.uuid4(),
                submission_id=submission.submission_id,
                evaluation_id=eval_record.evaluation_id,
                flag_reason=state.get("flag_reason", "LOW_CONFIDENCE_SCORE"),
                review_status="UNASSIGNED"
            )
            session.add(review_entry)

        session.commit()
        print(f"💾 Logged Evaluation '{eval_record.evaluation_id}' for Student '{student_user.full_name}' [{final_status}]")
...
if __name__ == "__main__":
    ...
    for role, uid in users.items():
        print(f"Role: {role:<15} | User ID: {uid}")